...
// --- Category ID mapping for collections page ---
const CATEGORY_ID_RANGES = {
  "Women":    [49, 64],
  "Men":      [1, 16],
  "Festive Gifts": [20, 32],
  "Madhubani": [33, 48],
  "Sterling Silver": [65, 80]
};

// Use shared products array from productdata.js
function getCheckedValues(selector) {
  return Array.from(document.querySelectorAll(selector + ":checked")).map(cb => cb.value);
}

function filterProducts() {
  let filtered = products.filter(p => p.id >= 1 && p.id <= 80);

  // Only category and price filters on collections page
  const cats = getCheckedValues(".filter-category");
  if (cats.length) {
    // Support multiple categories checked at once
    filtered = filtered.filter(p => {
      return cats.some(cat => {
        const [min, max] = CATEGORY_ID_RANGES[cat];
